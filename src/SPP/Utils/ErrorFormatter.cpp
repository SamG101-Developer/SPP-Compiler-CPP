...
auto SPP::Utils::ErrorFormatter::error(std::size_t start_pos, std::string &&message, std::string&& tag_message, const bool minimal) -> std::string {
    ...
    // auto err_line_as_string = token_stream
    //     | genex::views::slice(err_line_spos, err_line_epos)
    //     | genex::views::map([](const RawToken &tok) { return tok.tok_metadata; })
    ...
    // carets = carets | genex::views::slice(static_cast<std::size_t>(0), l1 - err_line_as_string.size());
